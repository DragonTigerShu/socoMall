    ...
    <!-- 正常滚动 -->
      <template  v-if="!sliderinit.loop">
        <template  v-for="item in pages">
          <div class="slider-item" :style="'background:url('+baseUrl+item.pic_ori+') center center no-repeat'" @click="backUrl(item.url)">
          <!-- {{item.title}} -->
          </div>
        </template>
      </template>
      ...
      <template  v-if="sliderinit.loop">
        <template v-if="sliderinit.infinite" v-for="(item, index) in pages" >
          <div v-if="index+sliderinit.infinite-pages.length>=0" class="slider-item" :style="'background:url('+baseUrl+item.pic_ori+') center center no-repeat'" @click="backUrl(item.url)"><!-- {{item.title}} --></div>
        </template>
        <template v-if="!sliderinit.infinite">
          <div class="slider-item" :style="'background:url('+baseUrl+pages[pages.length-1].pic_ori+') center center no-repeat'"  @click="backUrl(item.url)">{{pages[pages.length-1].title}} </div>
        </template>
        <!-- 无缝滚动copy最后项 -->
        <template  v-for="item in pages">
          <div class="slider-item" :style="'background:url('+baseUrl+item.pic_ori+') center center no-repeat'" @click="backUrl(item.url)"> {{item.title}}</div>
        </template>
        <!-- 无缝滚动copy第一项 -->
        <template v-if="sliderinit.infinite" v-for="(item, index) in pages" >
          <div v-if="index-sliderinit.infinite<0" class="slider-item" :style="'background:url('+baseUrl+item.pic_ori+') center center no-repeat' " @click="backUrl(item.url)"> {{item.title}}</div>
        </template>
        <template v-if="!sliderinit.infinite">
          <div class="slider-item" :style="'background:url('+baseUrl+pages[0].pic_ori+') center center no-repeat'" @click="backUrl(item.url)">{{pages[0].title}}</div>
        </template>
      </template>
    </div>
    <div class="slider-pagination slider-pagination-bullets">
      <template  v-for="n in pagenums">
        <span @click='slide(n-1)' class="slider-pagination-bullet" :class="n-1 == sliderinit.currentPage? 'slider-pagination-bullet-active':''">
          ...
          <i :class="n-1 == sliderinit.currentPage? 'slider-pagination-active':'slider-pagin'"></i>
        </span>
      </template>
    </div>
  </div>
...
export default {
  ...
  methods:{
    backUrl:function(evenlUrl){
      window.open('https://'+evenlUrl); 
    },
    swipeStart (e) {
      let that = this ;
    ...
    swipeEnd (e) {
      ...
      if (deltaTime > this.sliderinit.thresholdTime) {
        ...
        if ((deltaY > this.sliderinit.thresholdDistance)&&(Math.abs(deltaX) < this.sliderinit.thresholdDistance)) {
          ...
          if(this.basicdata.posheight!="0px")this.slide(this.sliderinit.currentPage);
            return
        }
      }
    },
