<template>
	<div class="info_aftersale_cation_one">
		<div class="info_aftersale w">
			<div class="soco_details_right_top info_aftersale_cation">
				<info_title :data="data">
				<h3 slot="info_header-title" class="info_header-title">售后申请</h3>
				</info_title> <!--换货申请-->
				<info_drawback :drawack="0" :infoIndex="0"></info_drawback>
			</div>
		</div>
		<div class="info_aftersale_bottom" v-if="data['order_info']">
			<h3>退款商品</h3>
			<Sale_list :data="data"></Sale_list> <!--信息列表-->
			<h3>服务类型</h3>
			<Sale_refund style="margin-bottom:20px" :recId="parseInt(recId)" :infoIndex="0"></Sale_refund>
			<div class="info_after_state">
				<div class="info_state_one">
					<h3>货物状态:</h3>
					<label>
						<span>
							<label for="y"><img :src="isReceive==0 ? imgchecked : imgUnChecked" ><input type="radio" id="y" name="shipping" style="display:none" value="0" v-model="isReceive">未收到货</label>
						</span>
						<span>
							<label for="n"><input type="radio" id="n" name="shipping" style="display:none" value="1" v-model="isReceive"><img :src="isReceive==1 ? imgchecked : imgUnChecked">已收到货</label>
						</span>
				...
				<div class="info_state_two">
					<h3>退货数量:</h3>
					<aN style="margin-left:20px;height:41px" :minusDis="minusDis" :addDis="addDis" :num="goodsNumber" @aNClickListen="aNClickListen"></aN>
				</div>
				<div class="info_state_two">
					<h3>退款原因:</h3>
				...
				<div class="info_state_three">
					<h3>退款金额:</h3>
					<div>
						<input type="text" :class="{'error':moneyError}" v-model="money" @focus="clearMoney" />
						<span class='error'>{{moneyErrorText}}</span>
						<span v-if="data['order_info']">*最大退款金额{{data['order_goods']['goods_price']*data['order_goods']['goods_number']}}元</span>
					</div>
				</div>
				<div class="info_state_four">
					<h3>退款说明:</h3>
					<textarea v-model='describe' placeholder="请在此描述问题"></textarea>
				</div>
				...
				<button type="button"  @click="cation_button()">提交</button>
...
<script>
	...
	var $this={}
	export default{
   	name:'info_afterSale_cation_one', //退货
   	...
   	methods:{
   		...
   		cation_button:function(){
   			if($this.reason==null||$this.reason==''){
   				...
   				$this.reasonErrorText='请选择退款原因';
   				$this.$emit("appListen","inform","请选择退款原因");
   				return false;
   			}
   			if($this.money==null||$this.money==''){
   				$this.moneyError=true;
   				$this.moneyErrorText='请填写退款金额';
   				$this.$emit("appListen","inform","请填写退款金额");
   				return false;
   			}
   			if(isNaN($this.money)||$this.money<0){
   				$this.moneyError=true;
   				$this.moneyErrorText='退款金额不规范';
   				$this.$emit("appListen","inform","退款金额不规范");
   				return false;
   			}
   			if($this.money>$this.data['order_goods']['goods_price']*$this.data['order_goods']['goods_number']){
   				$this.moneyError=true;
   				$this.moneyErrorText='退款金额不能大于商品金额';
   				$this.$emit("appListen","inform","退款金额不能大于商品金额");
   				return false;
   			}
		      ...
		      switch(parseInt(response.data.code)){
		      	case 1600100 :{
		      		...
		      		$this.$router.push({name:'info_afterSale_check',params:{'id':response.data.data.return_goods_id}});
		      		window.scrollTo(0,0);
		      		break;
		      	}
		      	case 1600103 :{
		      		$this.$emit("appListen","inform","无法重申售后");
		      		window.scrollTo(0,0);
		      		break;
		      	}
		      	case 1600108:{
		      		$this.$emit("appListen","inform","当前订单已超过7天售后期，无法提交申请");
		      		window.scrollTo(0,0);
		      		break;
		      	}
		      	case 888 :$this.$emit("appListen","loginAgain");break;
	...
	.info_aftersale_cation_one{
		...
		.info_aftersale_bottom{
			...
			.info_after_state{
				...
				.info_state_one,.info_state_two,.info_state_three,.info_state_four,.info_state_five{
					position:relative;
					>h3{
						line-height:normal;
						font-size: 20px;
