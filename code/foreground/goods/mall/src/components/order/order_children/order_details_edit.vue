  ...
  <div class="order_details_edit">
    <section>
      ...
      <div class="orderContent">
        <h1>收货地址</h1>
        <aL :reload="reload" :selectAddressId="selectAddressId" @selectListen="selectAddressListen" @appListen="appListen"></aL>
        <section class="group">
          <h3>支付方式</h3>
    ...
    <section>
      <div class="ordersList">
        ...
        <ul v-if="data" class="goodsListContent" @click="ulClickListen($event)">
    ...
    <section>
      <div class="orderDetail">
        <p>商品件数:<span v-if="detailData['order_goods_list']" >{{detailData['order_goods_list'].length}}件</span></p>
        <p>金额合计:<span>{{detailData['goods_amount']}}元</span></p>
        <p>活动优惠:<span>-{{discount}}元</span></p>
        <p>优惠券抵扣:<span>-{{discount}}元</span></p>
        <p>运费:<span>{{detailData['shipping_fee']}}元</span></p>
        <p>应付总额:<span style="font-size:26px">{{detailData['order_amount']}}<sub style="font-size:14px;font-weight:normal">元</sub></span></p>
      </div>
      <div class="orderGo">
        <button @click="nextTab">确认修改</button>
      </div>
    </section>
  </div>
</template>
<script>
  ...
  import goodsListContentOrderLi from '@/components/order/order_children/order_details_edit_goodsLi.vue';
  ...
  methods:{
      ...
      .then((response) => {
       ...
       switch(parseInt(response.data.code)){
         case 1100100 :{
          ...
          $this.selectAddressId=parseInt($this.detailData['address_id']);
          if($this.detailData['inv_type']==0){
            if($this.detailData['inv_payee']!=null&&$this.detailData['inv_payee']!=''){
              $this.FlagOne=true;
              $this.FlagTwo=true;
              $this.invPayee=$this.detailData['inv_payee'];
            }else{
              $this.FlagOne=true;
              $this.FlagTwo=false;
              $this.coName2=$this.detailData['co_name'];
              $this.coIdentNo2=$this.detailData['co_ident_no'];
            }
          }else{
            $this.FlagOne=false;
            $this.FlagTwo=false;
            $this.coName1=$this.detailData['co_name'];
            $this.coIdentNo1=$this.detailData['co_ident_no'];
            $this.depBankNo=$this.detailData['dep_bank_no'];
            $this.depBank=$this.detailData['dep_bank'];
            $this.regAddress=$this.detailData['reg_address'];
            $this.regNumber=$this.detailData['reg_number'];
          }
          break;
        }
    ...
    nextTab:function(){
      ...
      .then((response) => {
          ...
          switch(parseInt(response.data.code)){
            case 1100100 :{
              $this.$emit("appListen","inform","编辑信息成功");
              $this.$router.push({name: 'info_order_goods/order_details',params:{id:$this.detailData['order_id']}})
              window.scrollTo(0,0);
              break;
            }
            case 1100106:{
              $this.$emit("appListen","inform","已付款订单生效不可编辑");
              $this.$router.push({name: 'info_order_goods/order_details',params:{id:$this.detailData['order_id']}})
              window.scrollTo(0,0);
              break;
            }
    ...
    selectAddressListen:function($addressId){
      $this.selectAddressId=parseInt($addressId);
